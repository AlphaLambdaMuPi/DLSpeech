...
/*static PyObject *Model_Str(svms_ModelObject *self) {
  ...
  result = PyUnicode_FromString("Modely");
  return result;
  }*/
...
static PyObject *Model_getatupperbound(svms_ModelObject *self,void *closure) {
  return PyLong_FromLong(self->model->at_upper_bound);
}
...
static PyObject *Model_reduce(svms_ModelObject *self) {
  //return Py_BuildValue("O(iN)", self->ob_base.ob_type, self->type,
  //PySequence_Tuple((PyObject*)self));
  MODEL *m = self->model;
  return Py_BuildValue("O(NiiifNN)", self->ob_base.ob_type,

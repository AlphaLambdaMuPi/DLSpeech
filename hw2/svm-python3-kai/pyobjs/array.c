...
static PyObject *svms_GetItem(svms_ArrayObject *array, int i) {
  ...
  case AT_INT:
    return PyLong_FromLong(((int*)array->array)[i]);
  case AT_DOC:
...
static int svms_SetItem(svms_ArrayObject *array, PyObject*item,int i) {
  ...
  case AT_DOC: {
    ...
    if (array->ifree && doc_ob->ob_base.ob_refcnt==1 && doc_ob->ifree) {
...
static PyObject *Array_Str(svms_ArrayObject *self) {
  ...
  result = PyUnicode_FromFormat
    ("<%s of type %d, length %d>",self->ob_base.ob_type->tp_name,
...
static PyObject *Array_reduce(svms_ArrayObject *self) {
  return Py_BuildValue("O(iiN)", self->ob_base.ob_type, self->type, self->offset,
...
static PyObject* Array_subscript(svms_ArrayObject *self, PyObject *item) {
  if (PyLong_Check(item)) {
    return svms_GetItem(self, PyLong_AsLong(item));
